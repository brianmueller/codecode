import csv
import os
from curve import calculate_curve, calculate_curved_score
...
# --- Check if scores should be curved ---
curve = input("Do you want to curve the scores? (y/n): ").strip().lower()       
if curve == 'y':
...
for row in data_rows:
    ...
    student_score = float(row[7]) / float(row[6]) * 100
    class_section[student_name] = round(calculate_curved_score(slope, intercept, student_score), 1)
